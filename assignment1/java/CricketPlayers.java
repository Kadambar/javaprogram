...
public class CricketPlayers {
	...
	static void avg(int n, CricketPlayers c[]){
	try{
	for(int i=0;i<n;i++){
	...
	static void sort(int n, CricketPlayers c[]){
	String temp1;
	...
	CricketPlayers c[]=new CricketPlayers[n];
	for(int i=0;i<n;i++){
	c[i]=new CricketPlayers();
	c[i].get();
	}
	CricketPlayers.avg(n,c);
	CricketPlayers.sort(n, c);
	for(int i=0;i<n;i++){
	c[i].put();
	}
